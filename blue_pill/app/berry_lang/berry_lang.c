...
//     DISCARD_UNUSED(pPrompt);

//     static char buf[BERRY_MSG_BUFF_SIZE];
//     u32 len = xMessageBufferReceive(Berry_MessageBuff, buf, sizeof(buf), portMAX_DELAY);
//     if (len > 0)
//         return buf;
...
//         RTOS_Analyzer_DeleteTask(&BerryProcessIteractive_Handle);
// }
...
// 	X_CMD_ENTRY(BERRY_OPT_VM_EXEC, WSH_SHELL_OPT_STR(WSH_SHELL_OPT_ACCESS_EXECUTE, "-x", "--execute", "Execute berry script by filename (\"0:/xxx/yyy.be\") in separate VM")) \
...
static WSH_SHELL_RET_STATE_t ShellCmdBerry(const WshShellCmd_t* pcCmd, WshShell_Size_t argc,
    ...
    //                 RTOS_Analyzer_CreateTask(vTask_BerryLangInteractiveProcess, "berry-lang-vm",
    ...
    //                 bool vmState = BerryLangInteractive_GetState();
                    WSH_SHELL_PRINT_LEVEL(BerryVmInteractive_Handle ? WSH_SHELL_PRINT_SYS : WSH_SHELL_PRINT_ERR,
                                          "Berry VM create status: %s\r\n",
                                          BerryVmInteractive_Handle ? "success" : "fail");

                    if (!BerryVmInteractive_Handle)
                        return WSH_SHELL_RET_STATE_ERROR;
                } break;

    //             case BERRY_OPT_VM_EXEC: {
    //                 WshShell_Char_t filePath[WSH_SHELL_INTR_BUFF_LEN];
    //                 WshShellCmd_GetOptValue(&optCtx, argc, pArgv, sizeof(filePath),
    //                                         (WshShell_Size_t*)filePath);

    //                 u32 currTick = SYS_TICK_GET_MS_CNT();

    //                 bvm* bVm = be_vm_new();
    //                 if (!bVm) {
    //                     WSH_SHELL_PRINT_ERR("Can't create VM\r\n");
    //                     return WSH_SHELL_RET_STATE_ERROR;
    //                 }

    //                 int res = be_loadfile(bVm, filePath);
    //                 if (res != BE_OK)
    //                     be_dumpexcept(bVm);
    //                 else
    //                     res = be_pcall(bVm, 0);
    ...
    //                 WSH_SHELL_PRINT_LEVEL(res == BE_OK ? WSH_SHELL_PRINT_SYS : WSH_SHELL_PRINT_ERR,
    //                                       "Berry script executed with res %d in %d ms\r\n", res,
    //                                       SYS_TICK_GET_MS_CNT() - currTick);
